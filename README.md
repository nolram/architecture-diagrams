...
Six diagram families share the same validation → layout → render pipeline, selected by the spec's `type` field:
...
- **ER diagrams** (`type: er`): entities with attributes (PK/FK badges), weak entities (double border), and crow's-foot relationships (one / zero-or-one / many / zero-or-many) with identifying (solid) vs non-identifying (dashed) lines. Format: [`architecture-diagrams/reference/er-spec.md`](architecture-diagrams/reference/er-spec.md); runnable example: [`architecture-diagrams/reference/er.example.yaml`](architecture-diagrams/reference/er.example.yaml).
- **Timeline diagrams** (`type: timeline`): phased execution plans and roadmaps -- phases (gates and regular phases) with bullet items, laid out in a line with flow arrows and optional dependency labels. Gates render as diamonds, phases as rounded cards. Format: [`architecture-diagrams/reference/timeline-spec.md`](architecture-diagrams/reference/timeline-spec.md); runnable example: [`architecture-diagrams/reference/timeline.example.yaml`](architecture-diagrams/reference/timeline.example.yaml).
...
- `list_diagram_types` -- lists the registered engine types (`architecture`, `uml-class`, `uml-sequence`, `c4`, `er`, `timeline`) with a one-line description each.
...
src/
  ...
  engines/  diagram engines (architecture, uml-class, uml-sequence, c4, er, timeline) + registry
