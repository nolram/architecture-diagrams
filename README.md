...
Five diagram families share the same validation → layout → render pipeline, selected by the spec's `type` field:
...
- `list_diagram_types` -- lists the registered engine types (`architecture`, `uml-class`, `uml-sequence`, `c4`, `er`) with a one-line description each.
...
src/
  ...
  engines/  diagram engines (architecture, uml-class, uml-sequence, c4, er) + registry
